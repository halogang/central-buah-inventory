...
export function TabKeuangan() {
  ...
  const dailyColumns = [
    { key: "tanggal", label: "Tanggal" },
    {
      ...
      label: "Modal Masuk",
      align: "right" as const,
      render: (v: number | null) => <span className="text-primary font-medium">{formatRp(v)}</span>,
    },
    {
      key: "pengeluaran",
      label: "Pengeluaran",
      align: "right" as const,
      render: (v: number | null) => <span className="text-destructive font-medium">{formatRp(v)}</span>,
    },
    {
      key: "saldo",
      label: "Saldo",
      align: "right" as const,
      render: (v: number) => <span className="font-semibold">{formatRp(v)}</span>,
    },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <SummaryCard title="Total Pendapatan" value="Rp 58 Jt" subtitle="▲ +12.5% vs lalu" subtitleColor="success" icon={TrendingUp} />
        <SummaryCard title="Total Pengeluaran" value="Rp 32 Jt" subtitle="▼ -3.5% vs lalu" subtitleColor="destructive" icon={TrendingDown} />
        <SummaryCard title="Kas Bersih" value="Rp 26 Jt" subtitle="▲ +41.0%" subtitleColor="success" icon={DollarSign} />
      ...
      {/* Daily Cash Table */}
      <TableLaporan
        title="📅 Detail Modal Cash per Tanggal — Februari 2026"
        ...
        footer={
          <div className="grid grid-cols-3 gap-4 px-4 py-4">
            <div className="rounded-xl bg-primary/5 p-4 text-center">
              ...
              <p className="text-lg font-bold text-primary">Rp 58.000.000</p>
            </div>
            <div className="rounded-xl bg-destructive/5 p-4 text-center">
              <p className="text-xs text-muted-foreground">💸 Total Pengeluaran</p>
              <p className="text-lg font-bold text-destructive">Rp 32.000.000</p>
            </div>
            <div className="rounded-xl bg-muted p-4 text-center">
              <p className="text-xs text-muted-foreground">💰 Sisa Petty Cash</p>
              <p className="text-lg font-bold text-foreground">Rp 11.000.000</p>
            </div>
          </div>
        }
      />
